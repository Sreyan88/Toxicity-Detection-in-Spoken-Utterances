...
from mmi_module_only_audio import MMI_Model, MMI_Model_Single
...
def run(args, config, train_data, valid_data, test_data,trigger_data):
    ...
    accum_iter = 8
    stats_file_valid = open("/DeToxy/e2e/checkpoints/"  + "/" + "best_valid" + '_' + 'stats.txt', 'a', buffering=1)
    stats_file_test = open("/DeToxy/e2e/checkpoints/"  + "/" + "best_test" + '_' + 'stats.txt', 'a', buffering=1)
    stats_file_testt = open("/DeToxy/e2e/checkpoints/"  + "/" + "best_testt" + '_' + 'stats.txt', 'a', buffering=1)
    ...
    for epoch in range(epochs):
        ...
        if key_metric > best_metric:
            torch.save({'state_dict': model.state_dict(),'optimizer' : optimizer.state_dict()},"/speech/sreyan/DeToxy/FG_CME/checkpoints/MELD/" + "best" + '_' + "model.pt")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config", type=str, default="../DeToxy/e2e/config/iemocap-ours.yaml", help='configuration file path')
    parser.add_argument("--epochs", type=int, default=50, help="training epoches")
    parser.add_argument("--csv_path", type=str, default="../DeToxy/Dataset/Final.csv", help="path of csv")
    parser.add_argument("--save_path", type=str, default="./", help="report or ckpt save path")
    parser.add_argument("--data_path", type=str, default="../DeToxy/Dataset/Input/", help="report or ckpt save path")
    parser.add_argument("--data_path_bert", type=str, default="../DeToxy/e2e/numpy_bert/", help="report or ckpt save path")
    ...
    df_emotion=pd.read_csv("../DeToxy/Dataset/Transcripts/Final.csv",encoding='latin-1')
    df_emotion.rename(columns={'text':'Utterance','label2a':'Emotion'},inplace=True)
    
    df_emotion_train=df_emotion[df_emotion['split']==0]
    df_emotion_train.reset_index(drop= True, inplace= True)
    #df_emotion_train=df_emotion_train.head(15)

    df_emotion_valid=df_emotion[df_emotion['split']==1]
    df_emotion_valid.reset_index(drop= True, inplace= True)
    #df_emotion_valid=df_emotion_valid.head(10)

    df_emotion_test=df_emotion[df_emotion['split']==2]
    df_emotion_test.reset_index(drop= True, inplace= True)
    #df_emotion_test=df_emotion_test.head(10)

    df_emotion_trigger = pd.read_csv("../DeToxy/Dataset/Transcripts/trigger_test.csv",encoding='latin-1')
    df_emotion_trigger.rename(columns={'text':'Utterance','label2a':'Emotion'},inplace=True)
    df_emotion_trigger=df_emotion_trigger[df_emotion_trigger['split']==2]
    df_emotion_trigger.reset_index(drop= True, inplace= True)
    #df_emotion_trigger=df_emotion_trigger.head(10)
