...
def parse():
	...
	parser.add_argument('--train_data',help='Training Dataset Location',type = str,default="../data/",required = False)
	parser.add_argument('--test_data',help='Test Dataset Location',default="../DeToxy/Transcribed_Transcripts/",type = str,required=False)
...
if __name__=='__main__':
	...
	train_data_csv = pd.read_csv(args.train_data+"train.csv",encoding='latin-1')
	train_data_csv.rename(columns={'text':'Sentences','label2a':'Label'},inplace=True)
	
	valid_data_csv=pd.read_csv(args.test_data+'CommonVoice_Dev_transcripts.csv',encoding='latin-1')
	valid_data_csv.rename(columns={'text':'Sentences'},inplace=True)

	test_data_csv=pd.read_csv(args.test_data+'CommonVoice_Test_transcripts.csv',encoding='latin-1')
	test_data_csv.rename(columns={'text':'Sentences'},inplace=True)

	#Stats File
	stats_file = open("CommmonVoice_Dataset.txt", 'a', buffering=1)
